<template>
  <header>
    <select @change="changeMap($event)">
      <option v-for="map in maps" :value="map.id" :selected="activeMap == map.id" :key="map.id">
        {{ map.room }}
      </option>
    </select>
    <select @change="changeUser($event)">
      <option v-for="user in users" :value="user.id" :selected="activeUser == user.id" :key="user.id">
        {{ user.name }}
      </option>
    </select>
  </header>
